...
//! contracts and JSON (ADR 0006), and `altior-core` composes the three.
//! The manifests are embedded at compile time, so the test is
//! deterministic and never reads the working tree at runtime.
...
fn core_depends_only_on_domain_ipc_and_protocol() {
    assert_eq!(
        ...
        "altior-core composes the contract layers (ADR 0006)"
    );
}
